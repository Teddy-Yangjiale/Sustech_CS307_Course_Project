...
//    private final String url = "jdbc:postgresql://localhost:5433/project1?reWriteBatchedInserts=true";
//    private final String user = "postgres"; // 替换为你的用户名
...
public class AviationDataLoader {
    ...
    private Connection con = null;
    // 确保连接到了 WSL 中的 5433 端口，并开启底层批处理重写魔法
    private final String url = "jdbc:postgresql://localhost:5433/project1?reWriteBatchedInserts=true";
    ...
    public void loadCsvData(String filePath, String sql, DataBinder binder) {
        ...
        try (PreparedStatement pstmt = con.prepareStatement(sql);
            ...
            while ((line = br.readLine()) != null) {
                ...
                // CSV 按逗号分割，加上 -1 参数强制 Java 保留末尾的空列
                String[] data = line.split(",", -1);
                ...
                // 每 10000 条发送一次，防止内存溢出 (稍微调大一点以利用 reWriteBatchedInserts 优势)
                if (count % 10000 == 0) {
                    pstmt.executeBatch();
                    pstmt.clearBatch();
                }
            }
            pstmt.executeBatch(); // 执行剩余的尾巴数据
            con.commit();         // 正式提交写入磁盘
    ...
    public static void main(String[] args) {
        ...
        loader.loadCsvData("data/clean_airport.csv",
                ...
                (pstmt, data) -> {
                    ...
                    pstmt.setBigDecimal(7, new BigDecimal(data[6]));
        ...
        loader.loadCsvData("data/clean_route.csv",
                ...
                (pstmt, data) -> {
                    ...
                    // 防崩溃处理：直接传字符串或将其标记为未知对象，由 PG 原生驱动自动解析带时区的时间
                    pstmt.setObject(6, data[5], Types.OTHER);
                    pstmt.setObject(7, data[6], Types.OTHER);
        ...
        loader.loadCsvData("data/clean_flight.csv",
                ...
                (pstmt, data) -> {
                    ...
                    // 防崩溃处理：防止 Date.valueOf 解析 "YYYY-MM-DD +08" 崩溃
                    pstmt.setObject(3, data[2], Types.OTHER);
                }
        );
        ...
        loader.loadCsvData("data/clean_flight_cabin.csv",
                ...
                (pstmt, data) -> {
                    pstmt.setLong(1, Long.parseLong(data[0]));
                    pstmt.setString(2, data[1]);
                    pstmt.setBigDecimal(3, new BigDecimal(data[2]));
                    pstmt.setInt(4, Integer.parseInt(data[3]));
                }
        );
